...
from .serializers import AdminProductView, AdminProductStatusEdit, AddadminInquirySerializer, AdminInquiryListSerializer
...
class AdminInquiry(APIView):
    ...
    def get(self, request):
        ...
        serializer = AdminInquiryListSerializer(inquiry, many=True)
    ...
    def put(self, request):
        ...
        if request.data['answer'] != "":
            serializer = AddadminInquirySerializer(inquiry, data=request.data, partial=True)
            if serializer.is_valid():
                serializer.save(status=True)
                return Response({"message":"답변을 달았습니다.","data":serializer.data}, status=status.HTTP_201_CREATED)
            else:
